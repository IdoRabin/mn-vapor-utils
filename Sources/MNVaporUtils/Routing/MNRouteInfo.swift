...
public enum RouteProductType : String, Hashable, JSONSerializable {
    case unknown
    case webPage
    case apiResponse
}

public enum RouteInfoCodingKeys : String, Hashable, CodingKey, JSONSerializable {
    case ri_productType
    case ri_title
    case ri_desc
...
//public struct PersonStorageKey : ReqStorageKey {
...
public protocol MNRouteInfoable : AnyObject, JSONSerializable, Hashable, CustomStringConvertible {
    ...
    var requiredAuth : MNRouteAuth { get set }
    var httpMethods : Set<HTTPMethod>  { get set }
    func update(with other:(any MNRouteInfoable)?)
...
public extension MNRouteInfoable /* default inmplementation */ {
...
//        }
    ...
    func update(with other:(any MNRouteInfoable)?) {
        guard let other = other else {
            dlog?.note("update(with other:) - other MNRouteInfoable is nil!")
            return
        }
...
public class MNRouteInfo : MNRouteInfoable {
    
    public typealias CodingKeys = RouteInfoCodingKeys
    ...
    public var httpMethods = Set<HTTPMethod>()
    public var bodyStreamStrategy : HTTPBodyStreamStrategy = .collect
    ...
    */
    ...
    func update(with dict:[AnyHashable:Any]) {
        ...
        for key in CodingKeys.all {
            if let anything = dict[key.rawValue] {
                ...
                case .ri_required_auth:
                    if let auth = anything as? MNRouteAuth {
                ...
                case .ri_fullPath:
                    ...
                    self.fullPath = MNRoutes.normalizedRoutePath(pth)
    ...
    public required init(from decoder: Decoder) throws {
        ...
        for key in CodingKeys.all {
            ...
            case .ri_required_auth: self.requiredAuth   = try container.decode(MNRouteAuth.self, forKey:key)
...
//                }
    ...
    // MARK: Equatable
    public static func == (lhs: MNRouteInfo, rhs: MNRouteInfo) -> Bool {
...
extension Array where Element : MNRouteInfoable {
