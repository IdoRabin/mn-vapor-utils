...
extension Vapor.RoutesBuilder {
    ...
    ) -> Route? where Response: AsyncResponseEncodable
    {
        ...
        let responder = AsyncBasicResponder { request in
            ...
            if MNUtils.debug.IS_DEBUG && false {
        ...
        for method in methods.uniqueElements() {
            ...
            let mnRoute = route.mnRoute
            if let dict = dict, dict.count > 0 {
                mnRoute.update(with: dict) // add to the appRoute
                route.setting(routeInfoable: mnRoute) // add to the vapor.route.userInfo
            }
            
            appRoutes.registerRouteIfNeeded(mnRoute: mnRoute)
            lastRoute = route
        }
//
        lastRoute.debugValidateMNRoute(context: ".on(methods:path:body:info:\(dict?.count.description ?? "<nil>"):use:)")
...
//
            ...
            appRoutes.registerRouteIfNeeded(mnRoute: route.mnRoute)
            route.debugValidateMNRoute(context: ".redirects(methods:to:)")
            result.append(route)
        }
